...
# Data contains the header
read.table(file = '~/Desktop/mT/new-masterTable/targets_Hv3_mT.tsv', sep = '\t', header = TRUE) %>% 
  as_tibble() -> df_mT
df_mT
...
# Data contains the header
read.table(file = '~/Desktop/mT/new-masterTable/targets_Hv3_mT_with_tissues.tsv', sep = '\t', header = TRUE) %>% 
  as_tibble() -> df_mT_with_tissues
df_mT_with_tissues
...
```{r}
#pdf(file = "N_detected_targets.pdf", width = 8, height = 4)
df_targets %>% 
  filter(target_category=="longNonCoding") %>% 
  select(target_id, target_set) %>% 
  inner_join(x=.,
...
```{r}
#pdf(file = "detected_targets_percent.pdf", width = 8, height = 4)
df_targets %>% 
  filter(target_category=="longNonCoding") %>% 
  select(target_id, target_set) %>% 
  inner_join(x=.,
...
```{r}
#pdf(file = "N_detected_targets_percent2.pdf", width = 8, height = 4)
df_targets %>% 
  filter(target_category=="longNonCoding") %>% 
  select(target_id, target_set) %>% 
  inner_join(x=.,
...
# Targeted/detected chart - ale z naciskiem na % i z podziałem na tkanki
```{r}
#pdf(file = "N_detected_targets_percent2_tkanka.pdf", width = 8, height = 4)
df_targets %>% 
  filter(target_category=="longNonCoding") %>%
  select(target_id, target_set) %>% 
  inner_join(x=.,
            y=(df_mT_with_tissues %>%
                 filter(tissue=="Heart") %>% # Testis, CpoolA, Brain, Placenta, WBlood, Heart, TpoolA, Liver, iPSC
                 filter(technology=="ont") %>% # ont, pacBioSII
                 filter(stage=="Embryo") %>%
                 filter(capture_status=="post-capture") %>%
                 filter(bioReplicate==1) %>%
                 filter(techReplicate==1) %>%
                 #filter(endSupport=="noCageNoPolyASupported") %>% # polyAOnlySupported, noCageNoPolyASupported, cagePolyASupported, cageOnlySupported
  ...
  theme_minimal() +
  ylab('Percentage') +
  ggtitle(label = "Targeted to detected ratio for each catalogue (Brain)") +
  scale_fill_brewer(palette="Blues", direction = -1) +
  theme(axis.title.x=element_blank(),
        axis.text = element_text(colour = "black", size = 11),
        axis.ticks = element_line(colour = "black"),
...
# Testis, CpoolA, Brain, Placenta, WBlood, Heart, TpoolA, Liver, iPSC
...
# Junk below
## Fajny wykres, ale nie o to chodziło
```{r}
